...
def main():
    ...
    flight_number = fake_flight_data['flight_number']
    ...
    status = flight_status(flight_number, airline_code, flight_date, token)
    delay_prediction = flight_delay_prediction(airline_code, flight_number, origin, flight_date, token)
    check_in_info = check_in_links(airline_code, token)
    offers = flight_offers_search(origin, destination, departure_date, token)
    price_details = flight_offers_price(offers['data'][0]['id'], token) if 'data' in offers and offers['data'] else {}
    price_analysis = flight_price_analysis(origin, destination, departure_date, token)
    ...
    if delay_prediction.get('data') and any(pred['probability'] > 0.75 for pred in delay_prediction['data']):
        should_issue_reimbursement = True
